...
int clint_cold_ipi_init_range(int start, int end, struct clint_data *clint)
{
	...
	clint->ipi = (void *)clint->msip_addr;

	/* Update IPI hartid table */
	for (i = start; i < end; i++)
		clint_ipi_hartid2data[i] = clint;
...
#ifdef MANGO
	return mango_timer_value() + clint->time_delta;
#else
...
#ifdef MANGO
static inline void *mango_cmp_addr(struct clint_data *clint,
				   unsigned long hartid)
{
	unsigned int clusterid = MANGO_CLUSTER_ID(hartid);
	unsigned int coreid = MANGO_CORE_ID(hartid);

	return (void *)((unsigned long)(clint->time_cmp) +
...
#ifdef MANGO
	clint->time_wr(-1ULL, mango_cmp_addr(clint, target_hart));
#else
	clint->time_wr(-1ULL,
...
void clint_timer_event_start(u64 next_event)
{
	...
	clint->time_wr(next_event - clint->time_delta,
		       mango_cmp_addr(clint, target_hart));
#else
	clint->time_wr(next_event - clint->time_delta,
		       &clint->time_cmp[target_hart - clint->first_hartid]);
#endif
...
#ifdef MANGO
		sbi_printf("MANGO has no timer_delta_reference\n");
		sbi_hart_hang();
#endif
				...
				struct clint_data *reference)
{
	...
	clint->time_delta = 0;
	clint->time_val = NULL;
	clint->time_cmp = (u64 *)((void *)clint->mtimecmp_addr +
				  CLINT_TIME_CMP_OFF);
	...
	for (i = start; i < end; i++)
		clint_timer_hartid2data[i] = clint;
