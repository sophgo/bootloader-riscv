...
#define MANGO_PAGE_TABLE_BASE	MANGO_PA_BASE
...
{
	/* 4G memory */
	base[0] = mk_normal_pte(MANGO_PA_BASE + GB(0));
	base[1] = 0;
	base[2] = 0;
	base[3] = 0;

	/* 4G io */
	base[4] = mk_io_pte(MANGO_IO_BASE + GB(0));
	base[5] = mk_io_pte(MANGO_IO_BASE + GB(1));
	base[6] = mk_io_pte(MANGO_IO_BASE + GB(2));
	base[7] = mk_io_pte(MANGO_IO_BASE + GB(3));
...
sbi_hart_switch_mode_with_mmu(unsigned long arg0, unsigned long arg1,
		     ...
		     bool next_virt)
{
	unsigned long *mm_table = (void *)MANGO_PAGE_TABLE_BASE;
#if __riscv_xlen == 32
	...
	if (next_mode == PRV_S) {
		...
		if (coldboot_hartid < 0) {
			sbi_printf("internal error\n");
			sbi_hart_hang();
		}

		if (hartid == coldboot_hartid) {
	...
	/* TODO: for debug use, remote it if sbi is table */
	sbi_printf("hartid %lu, next_addr %016lx, arg0 %016lx, arg1 %016lx\n",
		   hartid, next_addr, arg0, arg1);
